  ...
  class Outer;
    ...
    class Inner;
      function void innerMethod(Outer h);
        ...
        outerLocalStaticProp = 0; // Legal, nested classes may access
                                  // local's in outer class
`ifdef NEGATIVE_TEST
        outerProp = 0; // Illegal, unqualified access to non-static outer
`endif
        h.outerProp = 0; // Legal, qualified access.
        h.outerLocalProp = 0; // Legal, qualified access and locals
                              // to outer class allowed.
      endfunction
    endclass
  endclass
